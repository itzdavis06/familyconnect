...
async function getChildren(token: string) {
  ...
  return members.filter((m: any) => m.isChild);
}
...
export default async function ChildProfile() {
  ...
  const children = token ? await getChildren(token.value) : [];
      ...
      <div className="mt-6 flex flex-wrap gap-4">
        {children.map((child: any) => (
          ...
          >
            ...
            <p className="mt-4 rounded-lg bg-gray-50 p-3 text-xs text-slate-500">
              Cannot log in or send messages. View-only within the family.
            </p>
          </div>
        ))}
      </div>
